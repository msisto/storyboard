...
// Storybook's file watcher (HMR) picks it up immediately.
...
function generateStoryFile(pascal: string, displayName: string, jsx: string): string {
...
app.post('/api/local-stories', async (req, res) => {
  const { name, jsx } = req.body as { name?: string; jsx?: string };
  if (!name || !jsx) return res.status(400).json({ error: 'name and jsx required' });
  ...
  try {
    ...
    fs.writeFileSync(filePath, generateStoryFile(pascal, name, jsx), 'utf-8');

...
export function buildLocalStoryFile(frame: Frame, name: string, stories: StorybookStory[]): string {
  ...
  // Collect human-readable labels for each component instance (no imports needed).
  const componentLabels = frame.components
    .filter((c) => c.visible)
    .map((c) => {
      const story = stories.find((s) => s.id === c.storybookId);
      return story ? `${story.title.split('/').pop()} · ${story.name}` : c.storybookId;
    });

  const frameStyle = frame.autoLayout
    ? `className="${buildAutoLayoutClassName(frame.autoLayout)}" style={{ width: ${frame.width}, height: ${frame.height}, background: '${frame.backgroundColor}' }}`
    : `style={{ position: 'relative', width: ${frame.width}, height: ${frame.height}, background: '${frame.backgroundColor}' }}`;
